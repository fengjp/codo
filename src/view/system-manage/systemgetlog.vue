...
<script>
  import {getGetLoglist} from '@/api/app'
  ...
  export default {
    ...
    methods: {
      getGetLoglist(page, limit, key, value, dateValue) {
        getGetLoglist(page, limit, key, value, dateValue).then(res => {
          if (res.data.code === 0) {
    ...
    mounted() {
      this.getGetLoglist(this.pageNum, this.pageSize)
    }
  }
</script>
