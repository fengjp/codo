...
import { createFile, getFileList } from '@/api/doc'
...
export default {
  data () {
    return {
      ...
      columns: [{ title: '报表名称', key: 'report_na', align: 'center' }],
      reportdata: [],
      columns2: [
        {
          title: '文件名称',
  ...
  mounted () {
    for (let i in this.reportList) {
      let _d = {}
      _d['report_na'] = this.reportList[i].label
      _d['report_val'] = this.reportList[i].value
      this.reportdata.push(_d)
    }
  }
}
</script>
