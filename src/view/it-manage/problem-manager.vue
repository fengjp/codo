<template>
  <div>
    ...
    <Card style="margin-top: 5px">
      ...
      <!--<Modal-->
      <!--v-model="modalTable.tableVisible"-->
      <!--:title="modalTable.tablelTitle"-->
      <!--:loading=true-->
      <!--:footer-hide=true-->
      <!--width="680"-->
      <!--&gt;-->
...
<script>
import { CaseAdd, getCaseList, caseDelete, getCaseFile } from '@/api/problem'
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        { title: '处理人', key: 'case_executor', align: 'center' },
        {
          ...
          align: 'center',
          render: (h, params) => {
            const roleTitle = params.row.case_details
            return h('div', [
              h('span', {
                style: {
  ...
  methods: {
    ...
    editModal (paramsRow, meth, mtitle) {
      ...
      this.getUserList()
      const loginUser = JSON.parse(sessionStorage.vuex).user.nickName
    ...
    getDictConfList () {
      getDictConfList().then(res => {
        if (res.data.code === 0) {
          const caseTypeList = JSON.parse(res.data.data.case_type_list)
          const caseObjList = JSON.parse(res.data.data.case_obj_list)
          const caseSourceList = JSON.parse(res.data.data.case_source_list)
          const caseStatusList = JSON.parse(res.data.data.case_status_list)
          const casePriorityList = JSON.parse(res.data.data.case_priority_list)

          if (caseTypeList) {
            this.allTypeList = caseTypeList
          }
          if (caseObjList) {
            this.allobjecList = caseObjList
          }
          if (caseSourceList) {
            this.allsourceList = caseSourceList
          }
          if (caseStatusList) {
            this.allstatusList = caseStatusList
          }
          if (casePriorityList) {
            this.allpriorityList = casePriorityList
          }
        } else {
          this.$Message.error(`${res.data.msg}`)
        }
